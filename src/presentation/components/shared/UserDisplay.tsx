...
export const UserDisplay = ({ onClear, size = 'md' }: UserDisplayProps) => {
  ...
  const labelStyles =
    size === 'sm' ? 'text-xs font-medium text-gray-dark' : 'text-sm font-medium text-gray-dark'
  ...
  const textStyles =
    size === 'sm' ? 'text-sm font-medium text-gray-dark' : 'font-medium text-gray-dark'
  ...
  return (
    <div className={containerStyles}>
      ...
      <div className={`flex items-center ${gapStyles}`}>
        <img src={avatar} alt={name} className={`${avatarSize} rounded-full object-cover`} />
